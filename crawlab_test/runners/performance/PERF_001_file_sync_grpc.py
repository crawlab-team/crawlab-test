...
def _verify_grpc_server(master, logger):
    ...
    if port_check["exit_code"] == 0:
        ...
        if "9666" in output and "LISTEN" in output:
            logger.info("   ✓ Port 9666 is bound and listening inside container")
            logger.debug(f"      {output}")
        else:
            logger.warning("   ⚠️  Port 9666 may not be properly bound")
            logger.warning(f"      Output: {output}")

    # Check process listening on port
    logger.info("   Checking for gRPC server process...")
    process_check = docker_utils.exec_command(
        master, r"ps aux | grep -i 'grpc\|crawlab' | grep -v grep | head -5", timeout=5
    )
    ...
    connectivity = verify_grpc_server_accessible(host="localhost", port=9666, max_retries=5, logger=logger)
    ...
    if not connectivity["accessible"]:
        ...
        docker_inspect = docker_utils.exec_command(master, "echo 'Container internal check passed'", timeout=5)
    ...
    rpc_test = test_grpc_connection(host="localhost", port=9666, spider_id="connectivity-test", logger=logger)
    ...
    try:
        ...
        comparison = {
            ...
            "improvements": {
                ...
                "throughput_speedup": round(http_result["duration"] / grpc_result["duration"], 2)
                if grpc_result["duration"] > 0
                else 0,
                "latency_improvement": round(http_result["avg_duration"] / grpc_result["avg_duration"], 2)
                if grpc_result["avg_duration"] > 0
                else 0,
            },
        }
        ...
        success = (
            grpc_result["success_rate"] >= min_success_rate
            and http_result["success_rate"] >= min_success_rate
            and grpc_result["json_errors"] == 0
            and http_result["json_errors"] == 0
            and grpc_result["scan_count"] < grpc_result["total"]  # Some deduplication happened
        )
        ...
        if success:
            ...
            logger.info(
                f"   • HTTP mode: {http_result['success_rate']:.1f}% success, {http_result['avg_duration']:.2f}s avg"
            )
            logger.info(
                f"   • gRPC mode: {grpc_result['success_rate']:.1f}% success, {grpc_result['avg_duration']:.2f}s avg"
            )
            ...
            if grpc_result["success_rate"] < 75.0 or http_result["success_rate"] < 75.0:
                logger.error("   • Low success rate (need ≥75%)")
            ...
            if grpc_result["scan_count"] >= grpc_result["total"]:
                logger.error(
                    f"   • No deduplication: {grpc_result['scan_count']} scans for {grpc_result['total']} requests"
                )
        logger.info("=" * 70)
...
def _test_concurrent_sync(master, workspace_base, spider_id, mode, num_concurrent, logger):
    ...
    if use_grpc:
        ...
        def single_directory_scan(request_id):
            ...
            duration = time.time() - req_start
            return {"id": request_id, "success": scan_result["exit_code"] == 0, "duration": duration}
