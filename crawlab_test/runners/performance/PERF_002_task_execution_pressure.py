...
class PressureTestRunner:
    ...
    def verify_environment(self) -> bool:
        ...
        try:
            ...
            if not nodes:
                ...
                worker_count = len([n for n in nodes if n.get('is_master') == False])
    ...
    def create_test_spider(self, cmd: Optional[str] = None) -> Optional[str]:
        ...
        spider_id, response = self.spider_helper.create_spider(
            ...
            name=spider_name,
            cmd=cmd,
            description="Automated pressure test spider"
        )
    ...
    def create_task_batch(self, batch_num: int, batch_size: int) -> Tuple[List[str], List[str], float]:
        ...
        for i in range(batch_size):
            try:
                ids, response = self.task_helper.create_task(
                    self.token,
                    self.test_spider_id,
                    priority=5
                )
    ...
    def run_load_test(self, num_tasks: int, num_workers: int, batch_size: int) -> Dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
                try:
                    ...
                    # Calculate progress
                    progress_pct = (i / num_batches) * 100
                    tasks_created = len(all_task_ids)
        ...
        # Display results
        self.log("=" * 60)
        self.log(f"📊 Task Creation Results:", "PROGRESS")
        self.log(f"  Target tasks: {num_tasks}")
        self.log(f"  Tasks created: {len(all_task_ids)} ({success_rate:.1f}%)", 
    ...
    def monitor_task_execution(self, task_ids: List[str], timeout: int = 600, 
                              sample_size: int = 50, check_interval: int = 10) -> Dict:
    ...
    def run_full_test(self, load_level: str = 'heavy', monitor: bool = True) -> Dict:
        ...
        try:
            ...
            if monitor and creation_results['task_ids']:
                ...
                execution_results = self.monitor_task_execution(
                    creation_results['task_ids'],
                    timeout=600
                )
    ...
    def _evaluate_results(self, results: Dict, load_level: str):
        ...
        if execution:
            ...
            if execution_rate >= threshold['execution']:
                self.log(f"✅ Execution success rate: {execution_rate:.1f}% (threshold: {threshold['execution']}%)", "SUCCESS")
            else:
                self.log(f"❌ Execution success rate: {execution_rate:.1f}% (threshold: {threshold['execution']}%)", "ERROR")
...
def main():
    ...
    parser.add_argument('--load', choices=['light', 'medium', 'heavy', 'extreme'], 
                       default='heavy', help='Load level to test')
