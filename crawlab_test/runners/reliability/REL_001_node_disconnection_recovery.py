...
class NodeDisconnectionTest:
    ...
    def __init__(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "exec", self.worker_container, "env"], capture_output=True, text=True, check=True
            )
    ...
    def disconnect_worker(self) -> bool:
        ...
        try:
            success = self.docker.simulate_network_disconnect(self.worker_container, self.network_name)
            if success:
    ...
    def reconnect_worker(self) -> bool:
        ...
        try:
            success = self.docker.simulate_network_reconnect(self.worker_container, self.network_name)
            if success:
    ...
    def wait_for_worker_ready(self, timeout: int = 120) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Polling for worker readiness (stability_period={stability_period}s, timeout={timeout}s, CI={is_ci})"
            )
                    ...
                    if reconnected_node:
                        ...
                        if is_ready:
                            ...
                            if stable_duration >= stability_period:
                                ...
                                self.logger.debug(
                                    f"Worker stable for {stable_duration:.1f}s, " f"waiting {remaining:.1f}s more..."
                                )
                        else:
                            ...
                            if first_active_time is not None:
                                ...
                                self.logger.debug(
                                    f"Worker not ready: active={is_active}, " f"enabled={is_enabled}, status={status}"
                                )
                    else:
                        if first_active_time is not None:
            ...
            try:
                nodes = self.api_client.get_nodes()
                self.logger.error(
                    f"Final node states at timeout: {[(n.get('name'), n.get('active'), n.get('status')) for n in nodes]}"
                )
            except:
                pass
    ...
    def verify_task_execution_on_reconnected_worker(self) -> bool:
        ...
        try:
            ...
            if not reconnected_node:
                self.logger.error("Cannot find reconnected worker in API")
                self.logger.error(
                    f"Search criteria: name='{self.worker_node_name}', container_id='{self.worker_container_id}', container='{self.worker_container}'"
                )
                self.logger.error(
                    f"Available nodes: {[(n.get('name'), n.get('key'), n.get('is_master'), n.get('status')) for n in nodes]}"
                )
                return False

            self.logger.info(
                f"Found reconnected node: {reconnected_node.get('name')} (key={reconnected_node.get('key')})"
            )
            ...
            self.logger.info(
                f"Node status verified: active={reconnected_node['active']}, enabled={reconnected_node.get('enabled', True)}, status={node_status}"
            )
            ...
            task_started = wait_for_condition(check_task_running, timeout=task_start_timeout, check_interval=3)
            ...
            task_completed = wait_for_condition(check_task_complete, timeout=60, check_interval=2)
