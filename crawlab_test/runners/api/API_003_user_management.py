...
from crawlab_test.helpers.api.user import UserHelper
...
def main():
    ...
    try:
        ...
        if user_data is None:
            print("✓ Non-existent user not found")
        else:
            print("⚠ Non-existent user request did not fail as expected")

        # Test 8: Update user email
        print("\n[Test 8] Updating user email...")
        success, response = user_helper.update_user(
            token, user1_id, username="test_user_api003", password="test123", email="updated@example.com"
        )
        if not success:
            raise Exception(f"Failed to update user email: {response}")
        print("✓ Email updated to: updated@example.com")

        # Test 9: Update username
        print("\n[Test 9] Updating username...")
        success, response = user_helper.update_user(
            token, user1_id, username="test_user_api003_updated", password="test123", email="updated@example.com"
        )
        if not success:
            raise Exception(f"Failed to update username: {response}")
        print("✓ Username updated to: test_user_api003_updated")
        ...
        if not success:
            print("✓ Duplicate username update rejected")
        else:
            print("⚠ Duplicate username update succeeded (backend may allow this)")
            # Revert the change
            user_helper.update_user(token, user1_id, username="test_user_api003_updated", password="test123")
        ...
        if old_token:
            print("⚠ Old password still works (token invalidation may not be immediate)")
        ...
        if not success:
            ...
            print("⚠ Empty password accepted (may be a validation issue)")
        ...
        if user_data is None:
            print("✓ Deleted user not found")
        else:
            print("⚠ Deleted user still accessible")
        ...
        if not success:
            print("✓ Non-existent user deletion returned error")
        else:
            print("✓ Non-existent user deletion handled (idempotent)")
