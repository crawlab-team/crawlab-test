...
def run_test():
    ...
    try:
        ...
        commit_data = git_helper.commit_git(token=token, git_id=git_id, message="Test commit from API")
        ...
        commit_files_data = git_helper.commit_git(
            token=token, git_id=git_id, message="Test commit specific files", files=["README.md"]
        )
        ...
        git_id_2, _ = git_helper.create_git(
            token=token, name="test-git-uncloned", url="https://github.com/crawlab-team/crawlab", auth_type="http"
        )
        ...
        try:
            ...
            commit_no_msg = git_helper.commit_git(token=token, git_id=git_id, message="")
