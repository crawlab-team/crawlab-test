...
def run_test():
    ...
    try:
        ...
        csv_export_id = db_helper.export_data(
            test_db_id, 
            "csv", 
            "test_collection"
        )
        ...
        check(isinstance(csv_export_id, str) and len(csv_export_id) > 0, 
              "Export ID is valid string")
        ...
        if csv_export_id:
            ...
            download_result = db_helper.download_export(test_db_id, "csv", csv_export_id)
        ...
        logger.info("\nStep 16: Start JSON export")
        json_export_id = db_helper.export_data(
            test_db_id,
            "json",
            "test_collection"
        )
        ...
        check(isinstance(json_export_id, str) and len(json_export_id) > 0,
              "JSON export ID is valid")
        ...
        nonexist_export = db_helper.export_data(
            test_db_id,
            "csv",
