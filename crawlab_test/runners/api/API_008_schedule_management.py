...
def main():
    ...
    try:
        ...
        test_spider_id, response = spider_helper.create_spider(
            token=token, name=f"test-spider-schedule-{int(time.time())}", cmd="python main.py"
        )
        ...
        if schedule_data:
            ...
            results.append(
                print_result(
                    schedule_data.get("name") == "Test Daily Schedule", f"Schedule name: {schedule_data.get('name')}"
                )
            )
            results.append(
                print_result(schedule_data.get("cron") == "0 0 * * *", f"Schedule cron: {schedule_data.get('cron')}")
            )
            results.append(
        ...
        results.append(print_result(has_schedules, f"Schedules listed: {len(schedules) if schedules else 0} found"))

        if has_schedules:
            found_our_schedule = any(s.get("_id") == schedule_id for s in schedules)
            results.append(print_result(found_our_schedule, "Created schedule found in list"))

        # Test pagination
        print_step(step, "Test schedule pagination")
        step += 1

        schedules_page1, response = schedule_helper.list_schedules(token, page=1, size=5)

        results.append(
            print_result(
                schedules_page1 is not None, f"Pagination works: {len(schedules_page1) if schedules_page1 else 0} items"
            )
        )
        ...
        filtered_schedules, response = schedule_helper.list_schedules(token, spider_id=test_spider_id)

        if filtered_schedules is not None:
            all_match_spider = all(s.get("spider_id") == test_spider_id for s in filtered_schedules)
            results.append(
                print_result(all_match_spider, f"Filter works: {len(filtered_schedules)} schedules for spider")
            )
        else:
        ...
        if current_schedule:
            ...
            replaced_schedule, response = schedule_helper.replace_schedule(
                token=token, schedule_id=schedule_id, schedule_data=current_schedule
            )
        ...
        if success:
            ...
            if schedule_data:
                results.append(
                    print_result(
                        schedule_data.get("enabled") is False, f"Schedule is disabled: {schedule_data.get('enabled')}"
                    )
                )
        ...
        if success:
            ...
            if schedule_data:
                results.append(
                    print_result(
                        schedule_data.get("enabled") is True, f"Schedule is enabled: {schedule_data.get('enabled')}"
                    )
                )
        ...
        if success:
            ...
            for sched_id in schedule_ids[1:]:
                ...
                if sched_data:
                    results.append(
                        print_result(sched_data.get("priority") == 10, f"Schedule {sched_id[:8]}... priority updated")
                    )
        ...
        if success:
            ...
            for sched_id in ids_to_delete:
                sched_data, _ = schedule_helper.get_schedule(token, sched_id)
                results.append(print_result(sched_data is None, f"Schedule {sched_id[:8]}... deleted"))
        ...
        if success:
            ...
            results.append(print_result(sched_data is None, "Deleted schedule no longer exists"))
        ...
        bad_schedule_id, response = schedule_helper.create_schedule(
            token=token, spider_id=test_spider_id, name="Bad Cron Schedule", cron="invalid cron", enabled=True
        )
        ...
        if invalid_spider_schedule is None:
            ...
            print("⚠️  System accepted schedule with non-existent spider (may validate at runtime)")
