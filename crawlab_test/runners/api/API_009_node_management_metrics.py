...
def main():
    ...
    try:
        ...
        results.append(print_result(has_nodes, f"Nodes listed: {len(nodes) if nodes else 0} found"))
        ...
        if len(master_nodes) == 0:
            ...
            results.append(print_result(True, f"Using node for testing: {master_node.get('name')} ({master_node_id})"))
        else:
            results.append(print_result(len(master_nodes) > 0, f"Master node(s) found: {len(master_nodes)}"))
        ...
        for field in required_fields:
            ...
            results.append(
                print_result(has_field, f"Node has '{field}': {master_node.get(field) if has_field else 'MISSING'}")
            )
        ...
        results.append(
            print_result(nodes_page1 is not None, f"Pagination works: {len(nodes_page1) if nodes_page1 else 0} items")
        )
        ...
        master_filter, response = node_helper.list_nodes(token, filter_dict={"is_master": True})
        ...
        if master_filter is not None:
            ...
            if len(master_filter) > 0:
                ...
                results.append(print_result(all_master, f"Master filter works: {len(master_filter)} master node(s)"))
            else:
        ...
        if master_node_id:
            ...
            if node_details:
                ...
                for field, value in detail_fields.items():
                    print(f"  - {field}: {value}")
                    results.append(print_result(value is not None, f"Node has {field}"))
        ...
        if master_node_id:
            ...
            updated_node, response = node_helper.update_node(
                token=token, node_id=master_node_id, updates={"description": "Updated test description"}
            )
            ...
            updated_node, response = node_helper.update_node(
                token=token, node_id=master_node_id, updates={"max_runners": 10}
            )
            ...
            if original_description is not None:
                node_helper.update_node(
                    token=token, node_id=master_node_id, updates={"description": original_description}
                )
        ...
        results.append(print_result(has_metrics, f"All node metrics: {len(all_metrics) if has_metrics else 0} nodes"))
        ...
        if has_metrics and master_node_id:
            ...
            if has_test_node_metrics:
                ...
                print("⚠️  Test node has no metrics yet (acceptable for new nodes)")
        ...
        if master_node_id:
            ...
            current_metrics, response = node_helper.get_node_current_metrics(token, master_node_id)
            ...
            if current_metrics:
                ...
                if not any(field in current_metrics for field in metric_fields):
                    ...
                    results.append(
                        print_result(len(current_metrics) > 0, f"Metrics data present: {len(current_metrics)} fields")
                    )
            else:
        ...
        if master_node_id:
            ...
            if time_metrics is not None:
                results.append(print_result(True, f"Time-range metrics retrieved: {len(time_metrics)} snapshots"))
                ...
                results.append(print_result(True, "Time-range metrics handled (may be no data in range)"))
            ...
            if time_metrics is not None:
                results.append(print_result(True, f"Time-range with params: {len(time_metrics)} snapshots"))
            else:
                # Acceptable if no data in range
                print("⚠️  No metrics data in time range (acceptable)")
                results.append(print_result(True, "Time-range params work (0 snapshots acceptable)"))
        ...
        results.append(print_result(node_data is None, "Invalid node ID handled"))
        ...
        is_handled = (
            metrics is None
            or (isinstance(metrics, dict) and metrics.get("node_id") == "000000000000000000000000")
            or (isinstance(metrics, dict) and len(metrics) == 0)
        )
        results.append(print_result(is_handled, "Invalid node metrics handled (zero object or None)"))
