...
def run_test():
    ...
    try:
        ...
        insert1_success = db_helper.insert_row(
            db_id, "test_tables_db", "test_users", {"username": "testuser1", "email": "test1@example.com"}
        )
        if insert1_success:
        ...
        insert2_success = db_helper.insert_row(
            db_id, "test_tables_db", "test_users", {"username": "testuser2", "email": "test2@example.com"}
        )
        if insert2_success:
        ...
        update_success = db_helper.update_rows(
            db_id, "test_tables_db", "test_users", {"email": "updated@example.com"}, {"username": "testuser1"}
        )
        if update_success:
        ...
        if updated_data and "data" in updated_data:
            ...
            found_updated = any(
                row.get("username") == "testuser1" and row.get("email") == "updated@example.com" for row in data_list
            )
            if found_updated:
        ...
        delete_success = db_helper.delete_rows(db_id, "test_tables_db", "test_users", {"username": "testuser2"})
        if delete_success:
        ...
        try:
            rename_success = db_helper.rename_table(db_id, "test_tables_db", "test_users", "users_renamed")
            if rename_success:
        ...
        try:
            modify_success = db_helper.modify_table(
                db_id, "test_tables_db", "test_users", columns=[{"name": "age", "type": "int"}]
            )
            if modify_success:
        ...
        try:
            invalid_data = db_helper.get_table_data(db_id, "test_tables_db", "nonexistent_table", page=1, size=10)
        ...
        if db_list and "data" in db_list:
            ...
            if not remaining:
                ...
                logger.warning("⚠ Test database still exists")
