...
def run_test():
    ...
    try:
        ...
        update_result = db_helper.update_database(mongo_id, {"description": "Updated description"})
        ...
        if len(paginated_list["data"]) <= 1:
            logger.info(
                f"✓ Pagination working: {len(paginated_list['data'])} items, total: {paginated_list.get('total', 0)}"
            )
            test_steps.append(("Pagination", True, "Page size respected"))
        else:
        ...
        batch_update_success = db_helper.batch_update_databases([mongo_id, mysql_id], {"description": "Batch updated"})
