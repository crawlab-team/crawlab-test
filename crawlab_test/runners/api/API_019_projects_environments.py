...
def run_test():
    ...
    try:
        ...
        if project1_id:
            ...
            updated_project = pe_helper.update_project(
                project1_id,
                {
        ...
        if project1_id:
            ...
            if project_detail:
                check(
                    "updated" in project_detail.get("description", "").lower()
                    or "updated" in project_detail.get("name", "").lower(),
        ...
        project2 = pe_helper.create_project(
            {"name": f"test_project_2_{int(time.time())}", "description": "Second test project"}
        )
        ...
        if len(project_ids) >= 2:
            ...
            batch_updated = pe_helper.batch_update_projects(
                project_ids[:2], {"description": "Batch updated description"}
            )
        ...
        logger.info("\nStep 12: Create environment")
        env1 = pe_helper.create_environment({"key": f"test_env_{int(time.time())}", "value": "Test environment value"})
        ...
        if env1_id:
            ...
            updated_env = pe_helper.update_environment(
                env1_id, {"key": f"test_env_updated_{int(time.time())}", "value": "Updated environment value"}
            )
        ...
        if env1_id:
            ...
            if env_detail:
                check(
                    "updated" in env_detail.get("value", "").lower() or "updated" in env_detail.get("key", "").lower(),
        ...
        env2 = pe_helper.create_environment(
            {"key": f"test_env_2_{int(time.time())}", "value": "Second test environment value"}
        )
        ...
        if len(environment_ids) >= 2:
            ...
            batch_updated = pe_helper.batch_update_environments(environment_ids[:2], {"value": "Batch updated value"})
        ...
        pe_helper.batch_update_projects(["invalid_id_1", "invalid_id_2"], {"description": "Test"})
        check(True, "Batch operation with invalid IDs handled")
