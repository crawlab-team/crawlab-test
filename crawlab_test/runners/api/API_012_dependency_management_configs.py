    ...
    try:
        ...
        if original_config:
            restore_success = update_dependency_config(
                token,
                "python",
                original_config['exec_cmd'],
                original_config['pkg_cmd'],
                original_config['pkg_src_url']
            )
            if restore_success:
        ...
        if setups_paged:
            logger.info(f"✓ Pagination working")
        ...
        install_version_success = install_dependency_config_setup(
            token, "python", mode="all", version="3.10"
        )
