...
from crawlab_test.helpers.api.git import GitHelper
...
def run_test():
    ...
    try:
        ...
        git_id, create_data = git_helper.create_git(
            token=token, name="test-git-repo", url="https://github.com/crawlab-team/crawlab-test-repo", auth_type="http"
        )
        ...
        print("\n[Step 5] Update git repository")
        update_data = git_helper.update_git(token=token, git_id=git_id, name="test-git-repo-updated")
        ...
        git_id_2, create_data_2 = git_helper.create_git(
            token=token, name="test-git-repo-2", url="https://github.com/crawlab-team/crawlab", auth_type="http"
        )
        ...
        batch_update_data = git_helper.batch_update_gits(
            token=token, git_ids=git_ids, description="Batch updated description"
        )
        ...
        try:
            invalid_git_id, invalid_create_data = git_helper.create_git(token=token, name="", url="")
        ...
        invalid_url_id, invalid_url_data = git_helper.create_git(
            token=token, name="test-invalid-url", url="not-a-valid-url"
        )
