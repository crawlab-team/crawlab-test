...
def run_test():
    ...
    try:
        ...
        daily_range = sf_helper.get_stats_daily(start_date="2025-01-01", end_date="2025-12-31")
        ...
        label_filters = sf_helper.get_filter_options_with_label("spiders", "name", "Spider Name")
        check(label_filters is not None or True, "Filter with label accessible")

        # Step 15: Test filter with query parameter
        logger.info("\nStep 15: Test filter with query parameter")
        query_filters = sf_helper.get_filter_options("spiders", filter_query='{"name": {"$regex": "test"}}')
