...
"""
        ...
        test_spider_id, response = spider_helper.create_spider(
            token=token, name=f"test-spider-logs-{int(time.time())}", cmd="python main.py", col_name="test_results"
        )
        ...
        file_saved, response = spider_helper.save_file(
            token=token, spider_id=test_spider_id, path="main.py", content=spider_script
        )

        results.append(print_result(file_saved, "Spider script saved"))
        ...
        task_ids, response = task_helper.create_task(token=token, spider_id=test_spider_id, mode="random")
        ...
        final_status, task_data = task_helper.wait_for_task(token=token, task_id=test_task_id, timeout=60, interval=2)

        results.append(print_result(final_status == "finished", f"Task status: {final_status or 'TIMEOUT'}"))
        ...
        results.append(print_result(has_logs, f"Logs retrieved: {len(logs) if logs else 0} characters"))

        if has_logs:
            print(f"📄 Log preview (first 200 chars):\n{logs[:200]}")

            # Verify expected content in logs
            expected_messages = ["Starting spider execution", "Processing data", "Spider execution completed"]
        ...
        try:
            ...
            results.append(print_result(response.status_code == 200, f"Log pagination test: {response.status_code}"))
        ...
        task_results, response = task_helper.get_task_results(token=token, task_id=test_task_id)
        ...
        if has_results and len(task_results) > 0:
            ...
            results.append(print_result(True, "Results endpoint works (0 items returned)"))
        ...
        try:
            ...
            results.append(print_result(page1_success, f"Results page 1: {response.status_code}"))
        ...
        results.append(print_result(edge_case_handled, "Invalid task ID handled gracefully"))

        task_results, response = task_helper.get_task_results(token=token, task_id=invalid_id)
        ...
        results.append(print_result(edge_case_handled, "Invalid task results handled gracefully"))
        ...
        try:
            ...
            results.append(
                print_result(response.status_code == 200, f"Latest logs (latest=true): {response.status_code}")
            )
            ...
            results.append(
                print_result(response.status_code == 200, f"Historical logs (latest=false): {response.status_code}")
            )
        except Exception as e:
