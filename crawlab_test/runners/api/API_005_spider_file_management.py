...
from crawlab_test.helpers.api import AuthHelper, SpiderHelper, CleanupHelper, APIAssertions
...
"""
    ...
    try:
        ...
        file_content, response = spider_helper.get_file(
            token=token,
            spider_id=spider_id,
            path="main.py"
        )
        ...
        if file_content != main_py_content:
            print_error(f"File content mismatch")
            print(f"Expected: {repr(main_py_content)}")
            print(f"Got: {repr(file_content)}")
            return False
        ...
        file_info, response = spider_helper.get_file_info(
            token=token,
            spider_id=spider_id,
            path="main.py"
        )
        ...
        files_list, response = spider_helper.list_files(
            token=token,
            spider_id=spider_id,
            path="/"
        )
        ...
        utils_files, response = spider_helper.list_files(
            token=token,
            spider_id=spider_id,
            path="/utils"
        )
        ...
        dir_info, response = spider_helper.get_file_info(
            token=token,
            spider_id=spider_id,
            path="utils"
        )
        ...
        copied_content, response = spider_helper.get_file(
            token=token,
            spider_id=spider_id,
            path="main_backup.py"
        )
        ...
        success, response = spider_helper.rename_file(
            token=token,
            spider_id=spider_id,
            old_path="main_backup.py",
            new_path="main_copy.py"
        )
        ...
        renamed_content, response = spider_helper.get_file(
            token=token,
            spider_id=spider_id,
            path="main_copy.py"
        )
