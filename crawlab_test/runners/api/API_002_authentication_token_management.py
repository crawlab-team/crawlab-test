...
from crawlab_test.helpers.api import APIAssertions, AuthHelper, CleanupHelper, UserHelper
...
def main():
    ...
    try:
        ...
        assertions.assert_field_equals(user_data, "username", "admin", "Username mismatch")
        ...
        assertions.assert_field_equals(token_data, "name", token_name, "Token name mismatch")

        print("✓ Token retrieved successfully")
        print("✓ Token data is correct")
        ...
        if "error" in response and "500" in str(response.get("error", "")):
            print("⚠ Known issue: /api/tokens GET returns 500 error")
            ...
            print("✓ Tokens listed successfully")
            print(f"✓ Found {len(tokens_list)} token(s)")
            print("✓ Created token found in list")
        ...
        assert token_data is None or "error" in response, "Token still exists after deletion"

        print("✓ Token no longer exists")
        print("✓ Appropriate error returned")
        ...
        if user_data is None:
            ...
            print("⚠ Token still valid after logout (JWT may not be invalidated)")
            print("  Note: This is acceptable for JWT-based auth")
