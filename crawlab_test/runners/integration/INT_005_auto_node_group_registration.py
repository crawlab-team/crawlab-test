...
class WorkerManager:
    ...
    def __init__(self, logger, network: str = None, image: str = None):
...
def main():
    ...
    try:
        ...
        if node_ids.count(worker1_id) != 1:
            logger.error(f"Worker {worker1_id} should appear exactly once in group, found {node_ids.count(worker1_id)} times")
            return 1
