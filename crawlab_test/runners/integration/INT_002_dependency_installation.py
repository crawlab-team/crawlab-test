...
class DependencyInstallationRobustnessTest:
    ...
    def __init__(self, worker_hint: str | None = None, keep_env: bool = False):
    ...
    def _select_worker_container(self, worker_hint: str | None) -> str:
    ...
    def _record_step(
        self, name: str, success: bool, message: str, stdout: str | None = None, stderr: str | None = None
    ) -> bool:
    ...
    def _pip(
        self, venv_path: str, args: list[str], timeout: int = 120, check: bool = False
    ) -> subprocess.CompletedProcess:
        pip_path = f"{venv_path}/bin/pip"
        cmd = [pip_path, "install", "--disable-pip-version-check", "--no-input", "--timeout", "60", *args]
        return self._exec(cmd, timeout=timeout, check=check)

    def _python(
        self, venv_path: str, args: list[str], timeout: int = 60, check: bool = False
    ) -> subprocess.CompletedProcess:
    ...
    def step_normal_installation(self) -> bool:
        ...
        try:
            ...
            message = (
                f"Installed requests==2.28.0 & beautifulsoup4==4.11.1 (requests version {verification.stdout.strip()})"
            )
        ...
        try:
            ...
            if conflict.returncode == 0:
                return self._record_step(
                    name, False, "Expected version conflict but pip succeeded", stdout=conflict.stdout
                )
