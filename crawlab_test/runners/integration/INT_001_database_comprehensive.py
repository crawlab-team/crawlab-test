...
class DatabaseIntegrationTest:
    ...
    def __init__(self):
        ...
        self.test_results = {"total": 0, "passed": 0, "failed": 0, "skipped": 0, "errors": []}
    ...
    def test_connection_management(self) -> bool:
        ...
        for db_type, config in TEST_DATABASES.items():
            ...
            if status in ["online", "success", "ok"]:
                ...
                if any(
                    indicator in str(error_msg).lower()
                    for indicator in [
                        "lookup",
                        "server misbehaving",
                        "connection refused",
                    ...
                    self.logger.warning(
                        f"  Start databases with: docker compose -f docker-compose.test.yml up -d {db_type}"
                    )
                self.record_result(f"{db_type}_connection", False, f"Connection failed: {error_msg}")
                all_passed = False
    ...
    def test_data_operations(self) -> bool:
        ...
        for db_type in ["mysql", "postgres"]:
            ...
            # INSERT
            insert_success = self.db_helper.insert_row(db_id, "test", table_name, {"name": "test_user", "value": 42})
            self.record_result(f"{db_type}_insert", insert_success, "" if insert_success else "Failed to insert row")
            ...
            update_success = self.db_helper.update_rows(
                db_id, "test", table_name, {"value": 100}, {"name": "test_user"}
            )
            self.record_result(f"{db_type}_update", update_success, "" if update_success else "Failed to update row")

            # DELETE
            delete_success = self.db_helper.delete_rows(db_id, "test", table_name, {"name": "test_user"})
            self.record_result(f"{db_type}_delete", delete_success, "" if delete_success else "Failed to delete row")
    ...
    def test_query_execution(self) -> bool:
        ...
        test_queries = {"mysql": "SELECT VERSION() as version", "postgres": "SELECT version()"}
