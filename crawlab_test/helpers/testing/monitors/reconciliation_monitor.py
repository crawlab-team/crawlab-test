...
class ReconciliationMonitor:
    ...
    def capture_initial_state(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Initial state captured: {initial_state['total_tasks']} tasks, "
    ...
    def monitor_single_cycle(self, expected_interval: int = 30) -> Dict[str, Any]:
        ...
        if changes["total_changes"] > 0:
            ...
            self.reconciliation_events.append(
                {"timestamp": datetime.now().isoformat(), "type": "cycle_detected", "changes": changes}
            )
        else:
    ...
    def capture_task_snapshot(self) -> Dict[str, Any]:
        ...
        snapshot = {"timestamp": datetime.now().isoformat(), "tasks": {}, "status_counts": defaultdict(int)}
...
def main():
    ...
    parser.add_argument("--wait-cycle", action="store_true", help="Wait for a single reconciliation cycle")
    parser.add_argument("--continuous", action="store_true", help="Monitor continuously for specified duration")
    parser.add_argument("--timeout", type=int, default=90, help="Timeout for waiting operations (seconds)")
    parser.add_argument("--duration", type=int, default=300, help="Duration for continuous monitoring (seconds)")
    parser.add_argument("--interval", type=int, default=30, help="Expected reconciliation interval (seconds)")
