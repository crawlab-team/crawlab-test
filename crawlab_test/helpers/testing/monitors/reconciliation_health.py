...
class ReconciliationHealthChecker:
    ...
    def check_task_state_consistency(self) -> bool:
        ...
        try:
            ...
            self.db.get_tasks_by_status(TASK_STATUS_NODE_DISCONNECTED)
            ...
            if inconsistencies > 0:
                self.logger.info(
                    f"ⓘ Found {inconsistencies} task state inconsistencies (normal for reconciliation testing)"
                )
            else:
    ...
    def check_periodic_reconciliation(self, wait_time: int = 90) -> bool:
        ...
        initial_count = self.db.count_tasks_by_status(TASK_STATUS_RUNNING) + self.db.count_tasks_by_status(
            TASK_STATUS_NODE_DISCONNECTED
        )
        ...
        while time.time() - start_time < wait_time:
            ...
            current_count = self.db.count_tasks_by_status(TASK_STATUS_RUNNING) + self.db.count_tasks_by_status(
                TASK_STATUS_NODE_DISCONNECTED
            )
    ...
    def check_reconciliation_interval(self, expected_interval: int = 30) -> bool:
        ...
        while time.time() - start_time < monitor_duration:
            ...
            if current_task:
                check_points.append(
                    {"time": time.time(), "status": current_task.get("status"), "error": current_task.get("error", "")}
                )
        ...
        for i in range(1, len(check_points)):
            if (
                check_points[i]["status"] != check_points[i - 1]["status"]
                or check_points[i]["error"] != check_points[i - 1]["error"]
            ):
                status_changes += 1

        if status_changes > 0:
            self.logger.info(
                f"✓ Detected {status_changes} reconciliation activities in {format_duration(monitor_duration)}"
            )
            return True
        else:
...
def main():
    ...
    parser.add_argument("--check-service", action="store_true", help="Check if service is running and operational")
    parser.add_argument(
        "--detailed", action="store_true", help="Run detailed health checks including periodic reconciliation"
    )
    parser.add_argument(
