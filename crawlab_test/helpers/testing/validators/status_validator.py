...
class StatusValidator:
    ...
    def create_uncertain_status_scenario(self, scenario_type: str) -> Dict[str, Any]:
        ...
        task_data = {"status": config["initial_status"], "node_id": test_node["_id"], "pid": config.get("pid", 12345)}
    ...
    def validate_conservative_behavior(self, scenario: Dict[str, Any], wait_time: int = 60) -> Dict[str, Any]:
        ...
        if not final_task:
            return {"valid": False, "error": "Task not found after reconciliation", "scenario": scenario["type"]}
        ...
        for forbidden_status in should_not_be:
            if final_task["status"] == forbidden_status:
                ...
                validation_result["issues"].append(f"Task incorrectly changed to forbidden status: {forbidden_status}")
        ...
        if scenario["type"] == "unreachable_worker" and error_msg and "disconnected" not in error_msg.lower():
            validation_result["issues"].append("Error message should indicate disconnection for unreachable worker")
        ...
        if validation_result["valid"] and validation_result["conservative_behavior"]:
            ...
            self.logger.error(
                f"✗ Conservative behavior violation in {scenario['type']}: " f"{validation_result['issues']}"
            )
    ...
    def check_no_false_abnormal_assignments(self) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Abnormal status analysis: {len(abnormal_tasks)} total, "
    ...
    def validate_status_transition_logic(self) -> Dict[str, Any]:
        ...
        recent_tasks = list(tasks_collection.find({"updated_at": {"$gte": one_hour_ago}}))
...
def main():
    ...
    parser.add_argument(
        "--check-conservative-logic", action="store_true", help="Run comprehensive conservative logic check"
    )
    parser.add_argument(
        "--check-abnormal-assignments", action="store_true", help="Check for false abnormal status assignments"
    )
    parser.add_argument("--check-transitions", action="store_true", help="Validate status transition logic")
