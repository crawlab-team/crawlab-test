...
class ProcessSimulator:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if any(keyword in cmdline.lower() for keyword in ["crawlab-server"]):
                    crawlab_processes.append({"pid": pinfo["pid"], "name": pinfo["name"], "cmdline": cmdline})
    ...
    def _simulate_dummy_process_crashes(self, count: int) -> Dict[str, Any]:
        ...
        for _i in range(count):
            ...
            task = self.db.create_test_task(status=TASK_STATUS_RUNNING, pid=proc.pid)
...
class SystemSimulator:
    ...
    def simulate_worker_disconnection(self, duration: int = 30) -> Dict[str, Any]:
        ...
        self.logger.info("Starting realistic worker disconnection simulation...")
    ...
    def simulate_mixed_scenario(self) -> Dict[str, Any]:
        ...
        results = {"simulation_type": "realistic_mixed_scenario", "scenarios": [], "total_duration": 0}
...
def main():
    ...
    parser.add_argument("--scenario", choices=["disconnect", "zombie", "mixed"], help="Type of scenario to simulate")
    parser.add_argument("--duration", type=int, default=30, help="Duration of disconnection simulation in seconds")
    parser.add_argument(
        "--check-sudo", action="store_true", help="Check if we have the required privileges for network simulation"
    )
    parser.add_argument("--cleanup-only", action="store_true", help="Only perform cleanup operations")
