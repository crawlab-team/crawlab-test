...
Examples:
  ...
  %(prog)s --custom --load heavy --monitor
    ...
    group.add_argument("--quick", action="store_true", help="Quick smoke test (light load, no monitoring)")
    group.add_argument("--standard", action="store_true", help="Standard capacity test (heavy load with monitoring)")
    group.add_argument("--stress", action="store_true", help="Full stress test (extreme load)")
    group.add_argument("--custom", action="store_true", help="Custom configuration")

    parser.add_argument("--load", choices=["light", "medium", "heavy", "extreme"], help="Load level (for custom mode)")
    parser.add_argument("--monitor", action="store_true", help="Monitor execution (for custom mode)")
    ...
    runner_path = os.path.join(script_dir, "..", "runners", "scheduler", "SCH-002_task_execution_pressure_test.py")
