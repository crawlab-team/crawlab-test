...
class TaskBaselineManager:
    ...
    def create_test_spiders(self, count: int = 3) -> List[Dict[str, Any]]:
        ...
        for _i, spider_type in enumerate(spider_types[:count]):
...
def main():
    ...
    parser.add_argument("--cleanup", action="store_true", help="Clean up existing baseline")
    parser.add_argument("--validate", action="store_true", help="Validate existing baseline")
    ...
    parser.add_argument(
        "--types",
        choices=["mixed", "running", "pending", "completed"],
        default="mixed",
        help="Types of tasks to create",
    )
