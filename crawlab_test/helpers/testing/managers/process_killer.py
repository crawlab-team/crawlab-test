...
from crawlab_test.helpers.infrastructure.utils import load_config, setup_logging
...
class ProcessKiller:
        ...
        for i in range(count):
            ...
            try:
                ...
                test_task = self.db.create_test_task(
                    status=TASK_STATUS_RUNNING, node_id=test_node["_id"], pid=proc.pid, cmd=" ".join(cmd)
                )
    ...
    def kill_processes_by_pattern(
        self, pattern: str, count: Optional[int] = None, method: str = "SIGKILL"
    ) -> List[Dict[str, Any]]:
    ...
    def simulate_process_crashes(self, crash_scenarios: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        results = {"scenarios": {}, "total_killed": 0, "zombie_tasks_created": 0}
    ...
    def verify_zombie_state(self) -> Dict[str, Any]:
        ...
        for task in running_tasks:
            pid = task.get("pid", 0)
            if pid and pid > 0 and not self.process_manager.is_process_running(pid):
                zombie_tasks.append(
                    {
...
def main():
    ...
    parser.add_argument(
        "--target",
        choices=["running-tasks", "pattern", "pids", "simulate-crashes"],
        default="running-tasks",
        help="Target for process killing",
    )
    parser.add_argument(
        "--method",
        choices=["SIGKILL", "SIGTERM", "SIGSEGV", "SIGINT"],
        default="SIGKILL",
        help="Method to kill processes",
    )
    parser.add_argument("--count", type=int, default=3, help="Number of processes to kill")
    parser.add_argument("--pattern", default="crawlab", help="Process pattern to match (for pattern target)")
    parser.add_argument("--pids", nargs="+", type=int, help="Specific PIDs to kill (for pids target)")
    parser.add_argument("--verify", action="store_true", help="Verify zombie state after killing")
    parser.add_argument("--cleanup", action="store_true", help="Clean up test processes")
    ...
    try:
        ...
        if args.target == "running-tasks":
            ...
            logger.info(
                f"Simulated crashes: {results['total_killed']} processes killed, "
