...
from services import sync_service_pb2
from services import sync_service_pb2_grpc


def verify_grpc_server_accessible(host: str = 'localhost', port: int = 9666, max_retries: int = 10, logger=None) -> Dict[str, any]:
    """
...
def test_grpc_connection(host: str = 'localhost', port: int = 9666, spider_id: str = 'test', logger=None) -> Dict[str, any]:
    """
    ...
    try:
        ...
        request = sync_service_pb2.FileSyncRequest(
            spider_id=spider_id,
            path='',
            node_key='connectivity-test'
        )
        ...
        result['success'] = True
        if logger:
            logger.info(f"✓ gRPC connection test successful")
