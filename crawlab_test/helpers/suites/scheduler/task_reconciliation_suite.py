...
from crawlab_test.helpers.infrastructure.utils import setup_logging, TestMetrics, format_duration, load_config
...
class TaskReconciliationTestSuite:
    ...
    def run_helper_script(self, script_name: str, args: List[str] = None, 
    ...
    def step_3_simulate_zombie_tasks(self) -> bool:
        ...
        result = self.run_helper_script(
            'process-killer.py',
            ['--target', 'running-tasks', '--method', 'SIGKILL', '--count', '3', '--verify']
        )
    ...
    def step_8_continuous_monitoring(self) -> bool:
        ...
        result = self.run_helper_script(
            'reconciliation-monitor.py',
            ['--continuous', '--duration', '180', '--interval', '30']
        )
    ...
    def step_9_final_consistency_check(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Final state: {len(running_tasks)} running, "
    ...
    def cleanup_test_environment(self) -> bool:
        ...
        try:
            # Clean up test data
            cleanup_result = self.run_helper_script(
                '../common/cleanup.py',
                ['--reconciliation-test-data', '--temp-files']
            )
            
            # Reset any offline nodes (best effort)
            try:
                nodes = self.api.get_nodes()
                offline_nodes = [node for node in nodes 
                               if node.get('status') != 'online' and 'test' in node.get('key', '').lower()]
...
def main():
    ...
    parser.add_argument('--full-suite', action='store_true',
                       help='Run the complete reconciliation test suite')
    parser.add_argument('--test', choices=[
        'zombie-detection', 'worker-reconnection', 'grpc-protocol',
        'conservative-logic', 'performance'
    ...
    try:
        ...
        if args.full_suite:
            ...
            if args.test == 'zombie-detection':
                ...
                success = (test_suite.step_2_establish_baseline() and
                          test_suite.step_3_simulate_zombie_tasks() and
                          test_suite.step_4_wait_for_reconciliation())
                test_suite.metrics.end_step(success)
                
            elif args.test == 'worker-reconnection':
                test_suite.metrics.start_step("Worker Reconnection")
                success = (test_suite.step_5_simulate_worker_disconnection() and
                          test_suite.step_7_test_worker_reconnection())
                test_suite.metrics.end_step(success)
