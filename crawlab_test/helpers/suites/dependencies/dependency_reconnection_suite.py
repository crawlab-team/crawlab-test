...
class DependencyReconnectionTest:
    def __init__(self, master_url: str = "http://localhost:8080", api_token: Optional[str] = None):
        ...
        self.test_results = {"start_time": time.time(), "steps": [], "errors": [], "status": "unknown"}
    ...
    def step_1_verify_initial_state(self) -> bool:
        ...
        while time.time() < deadline and not self.target_worker:
            ...
            worker_nodes = [n for n in nodes if self._is_worker_node(n) and (n.get("status") or "").lower() == "online"]
    ...
    def step_2_install_test_dependencies(self) -> bool:
        ...
        for package in self.test_packages:
            success = self._install_dependency(package["lang"], package["name"], package["version"])
        ...
        self._record_step("install_test_dependencies", True, f"Installed {len(installed_packages)} test dependencies")
        return True
    ...
    def step_3_verify_dependency_sync(self) -> bool:
        ...
        self._record_step("verify_dependency_sync", True, f"Found {found_packages} dependencies in sync")
        return True
    ...
    def step_5_verify_disconnection_handling(self) -> bool:
        ...
        self._install_dependency(
            test_package["lang"], test_package["name"], test_package["version"], expect_failure=True
        )

        self._record_step("verify_disconnection_handling", True, "System handled disconnection gracefully")
        return True
    ...
    def step_7_verify_reconnection_sync(self) -> bool:
        ...
        self._record_step("verify_reconnection_sync", True, "Dependency sync restored after reconnection")
        return True
    ...
    def step_8_test_post_reconnection_operations(self) -> bool:
        ...
        success = self._install_dependency(test_package["lang"], test_package["name"], test_package["version"])
        ...
        self._record_step(
            "test_post_reconnection_operations", True, "Dependency operations work correctly after reconnection"
        )
        return True
    ...
    def _wait_for_dependency_sync(self, timeout: int = 60) -> bool:
        ...
        return wait_for_condition(lambda: self._check_dependency_sync_status(), timeout=timeout, check_interval=5)
    ...
    def _record_step(self, step_name: str, success: bool, message: str):
        ...
        self.test_results["steps"].append(
            {"step": step_name, "success": success, "message": message, "timestamp": time.time()}
        )
...
def main():
    ...
    parser.add_argument("--ci", action="store_true", help="Running in CI environment (for compatibility)")
