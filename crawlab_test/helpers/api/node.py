...
class NodeHelper:
    ...
    def list_nodes(
        self, token: str, page: int = 1, size: int = 10, filter_dict: Optional[Dict] = None
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/nodes", headers={"Authorization": f"Bearer {token}"}, params=params, timeout=10
            )
    ...
    def get_node(self, token: str, node_id: str) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/nodes/{node_id}", headers={"Authorization": f"Bearer {token}"}, timeout=10
            )
    ...
    def update_node(self, token: str, node_id: str, updates: Dict) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Update node failed with status {response.status_code}",
    ...
    def get_node_time_range_metrics(
        self,
        token: str,
        node_id: str,
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Get node time-range metrics failed with status {response.status_code}",
                    "response": response.text,
                }
