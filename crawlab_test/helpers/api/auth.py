...
class AuthHelper:
    ...
    def login(self, username: str = "admin", password: str = "admin") -> Tuple[Optional[str], Optional[Dict]]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/login", json={"username": username, "password": password}, timeout=10
            )
    ...
    def logout(self, token: str) -> Tuple[bool, Optional[Dict]]:
        ...
        """
        try:
            response = requests.post(f"{self.base_url}/logout", headers=self.get_auth_headers(token), timeout=10)
    ...
    def verify_token(self, token: str) -> bool:
        ...
        try:
            ...
            response = requests.get(f"{self.base_url}/users/me", headers=self.get_auth_headers(token), timeout=10)
    ...
    def create_token(self, token: str, name: str, role: Optional[str] = None) -> Tuple[Optional[str], Optional[Dict]]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/tokens", headers=self.get_auth_headers(token), json=payload, timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Token creation failed with status {response.status_code}",
    ...
    def get_token(self, token: str, token_id: str) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/tokens/{token_id}", headers=self.get_auth_headers(token), timeout=10
            )
    ...
    def list_tokens(self, token: str) -> Tuple[Optional[list], Optional[Dict]]:
        ...
        """
        try:
            response = requests.get(f"{self.base_url}/tokens", headers=self.get_auth_headers(token), timeout=10)
