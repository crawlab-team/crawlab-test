...
class UserHelper:
    ...
    def create_user(
        ...
        **kwargs,
        ...
        """
        try:
            payload = {"data": {"username": username, "password": password, **kwargs}}
            if email:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"User creation failed with status {response.status_code}",
    ...
    def get_me(self, token: str) -> Tuple[Optional[Dict], Optional[Dict]]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/users/me", headers={"Authorization": f"Bearer {token}"}, timeout=10
            )
    ...
    def list_users(self, token: str) -> Tuple[Optional[List], Optional[Dict]]:
        ...
        """
        try:
            response = requests.get(f"{self.base_url}/users", headers={"Authorization": f"Bearer {token}"}, timeout=10)
