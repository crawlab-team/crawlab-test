...
class ScheduleHelper:
    ...
    def create_schedule(
        ...
        """
        try:
            payload = {"spider_id": spider_id, "name": name, "cron": cron, "enabled": enabled, **kwargs}
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Schedule creation failed with status {response.status_code}",
    ...
    def get_schedule(self, token: str, schedule_id: str) -> tuple[dict | None, dict | None]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/schedules/{schedule_id}", headers={"Authorization": f"Bearer {token}"}, timeout=10
            )
    ...
    def list_schedules(
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/schedules", headers={"Authorization": f"Bearer {token}"}, params=params, timeout=10
            )
    ...
    def update_schedule(self, token: str, schedule_id: str, updates: dict) -> tuple[dict | None, dict | None]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Update schedule failed with status {response.status_code}",
                    "response": response.text,
                }
    ...
    def replace_schedule(self, token: str, schedule_id: str, schedule_data: dict) -> tuple[dict | None, dict | None]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Replace schedule failed with status {response.status_code}",
                    "response": response.text,
                }
    ...
    def run_schedule(self, token: str, schedule_id: str, **kwargs) -> tuple[list[str] | None, dict | None]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {
                    "error": f"Run schedule failed with status {response.status_code}",
                    "response": response.text,
                }
    ...
    def batch_update_schedules(self, token: str, schedule_ids: list[str], updates: dict) -> tuple[bool, dict | None]:
        ...
        """
        try:
            payload = {"ids": schedule_ids, "update": updates}
