...
class StatsFiltersAPIHelper:
    ...
    def get_stats_overview(self) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            response = requests.get(f"{self.base_url}/stats/overview", headers=self._get_headers(), timeout=10)
    ...
    def get_stats_tasks(self) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            response = requests.get(f"{self.base_url}/stats/tasks", headers=self._get_headers(), timeout=10)
            response.raise_for_status()
            result = response.json()
            return result.get("data")
        except Exception as e:
    ...
    def get_filter_options_with_value(
        self, collection: str, value: str, filter_query: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_filter_options_with_label(
        self, collection: str, value: str, label: str, filter_query: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """
