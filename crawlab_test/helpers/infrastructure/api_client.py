...
class CrawlabAPIClient:
    ...
    def get_node_by_id(self, node_id: Union[str, ObjectId]) -> Optional[Dict[str, Any]]:
        """Get node by ID"""
        response = self._request("GET", f"/nodes/{node_id!s}")
    ...
    def get_task_by_id(self, task_id: Union[str, ObjectId]) -> Optional[Dict[str, Any]]:
        """Get task by ID"""
        response = self._request("GET", f"/tasks/{task_id!s}")
        if response.status_code == 404:
            return None
        data = self._get_json_response(response)
        return data.get("data")
    ...
    def update_task(self, task_id: Union[str, ObjectId], task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = self._request("PUT", f"/tasks/{task_id!s}", json=task_data)
        data = self._get_json_response(response)
        return data.get("data")

    def cancel_task(self, task_id: Union[str, ObjectId], force: bool = False) -> bool:
        """Cancel a running task"""
        response = self._request("POST", f"/tasks/{task_id!s}/cancel", json={"force": force})
        return response.status_code == 200

    def restart_task(self, task_id: Union[str, ObjectId]) -> Dict[str, Any]:
        """Restart a task"""
        response = self._request("POST", f"/tasks/{task_id!s}/restart")
        data = self._get_json_response(response)
        return data.get("data")

    def delete_task(self, task_id: Union[str, ObjectId]) -> bool:
        """Delete a task"""
        response = self._request("DELETE", f"/tasks/{task_id!s}")
        return response.status_code == 200
    ...
    def force_reconcile_task(self, task_id: Union[str, ObjectId]) -> bool:
        ...
        response = self._request("POST", f"/tasks/{task_id!s}/reconcile")
        return response.status_code == 200
    ...
    def get_spider_by_id(self, spider_id: Union[str, ObjectId]) -> Optional[Dict[str, Any]]:
        """Get spider by ID"""
        response = self._request("GET", f"/spiders/{spider_id!s}")
        if response.status_code == 404:
            return None
        data = self._get_json_response(response)
        return data.get("data")
    ...
    def wait_for_task_status(
        self, task_id: Union[str, ObjectId], expected_status: str, timeout: int = 60, poll_interval: float = 2.0
    ) -> bool:
    ...
    def wait_for_node_status(
        self, node_key: str, expected_status: str, timeout: int = 60, poll_interval: float = 2.0
    ) -> bool:
