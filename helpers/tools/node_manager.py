...
class NodeManager:
    ...
    def _find_worker_container(self, include_stopped: bool = False, target_worker_node: Dict = None) -> Optional[str]:
            ...
            if target_worker_node:
                ...
                for container in containers:
                    if 'worker' in container.lower():
                        ...
                        if (node_key and any(part in container for part in node_key.split('-')[:2])) or \
                           (node_name and node_name.lower() in container.lower()) or \
                           (node_key and node_key[-4:] in container):  # Last 4 chars
    ...
    def _disconnect_network(self, node: Dict) -> bool:
        ...
        commands = [
            f"sudo iptables -A INPUT -s {node_ip} -j DROP",
            f"sudo iptables -A OUTPUT -d {node_ip} -j DROP"
        ]
    ...
    def _reconnect_process(self, node_name: str) -> bool:
        ...
        commands = [
            f"systemctl start crawlab-worker-{node_name}",
            f"docker start crawlab-worker-{node_name}"
        ]
    ...
    def _reconnect_docker(self, node_name: str) -> bool:
        ...
        if not worker_container:
            print(f"Could not find running worker container for reconnection")
            return False
...
def main():
    ...
    disconnect_parser.add_argument('--method', choices=['network', 'process', 'docker'], 
                                  default='network', help='Disconnection method')
