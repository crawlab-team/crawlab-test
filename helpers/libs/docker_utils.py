...
class DockerUtils:
    ...
    def find_crawlab_containers(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not containers:
                result = subprocess.run([
                    'docker', 'ps', '--format', 'json',
                    '--filter', 'ancestor=crawlabteam/crawlab-pro',
                    '--filter', 'status=running'
                ], capture_output=True, text=True, timeout=10)
    ...
    def find_master_container(self) -> Optional[Dict[str, Any]]:
        ...
        for container in containers:
            ...
            if any(keyword in names.lower() for keyword in ['master', '_master_', 'test_master']):
                return container
            elif 'master' in image.lower():
                return container
    ...
    def get_network_info(self, network_name: str = None) -> Dict[str, Any]:
    ...
    def simulate_network_disconnect(self, container_name: str, network_name: str = None) -> bool:
