...
class TaskHelper:
    ...
    def create_task(self, token: str, spider_id: str, cmd: Optional[str] = None,
                   mode: str = "random", priority: int = 5, **kwargs) -> Tuple[Optional[List[str]], Optional[Dict]]:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return None, {"error": f"Task creation failed with status {response.status_code}", "response": response.text}
    ...
    def wait_for_task(self, token: str, task_id: str, timeout: int = 60, 
                     interval: int = 1) -> Tuple[Optional[str], Optional[Dict]]:
        """
    ...
    def restart_task(self, token: str, task_id: str) -> Tuple[Optional[str], Optional[Dict]]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/tasks/{task_id}/restart",
                headers={"Authorization": f"Bearer {token}"},
                timeout=10
            )
