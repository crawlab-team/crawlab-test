...
class FileSyncTester:
    ...
    def compare_modes(self, concurrent_tasks: int = 10, iterations: int = 3) -> Dict:
        ...
        improvements = {
            ...
            "latency_reduction_percent": round((1 - grpc_avg.get('avg_duration_sec', 0) / http_avg.get('avg_duration_sec', 1)) * 100, 2),
            "deduplication_ratio": f"1:{concurrent_tasks}" if grpc_avg.get('deduplication_working') else "none"
        }
        
        report = {
            "http_average": http_avg,
            "grpc_average": grpc_avg,
    ...
    def run_full_suite(self) -> Dict:
        ...
        # Test 1: Basic gRPC
        results['tests']['basic_grpc'] = {
            "passed": self.test_grpc_basic()
        }
        time.sleep(5)
        ...
        if self.master_container:
            ...
            try:
                self.docker.exec_in_container(
                    ...
                    ['sh', '-c', f'export CRAWLAB_SYNC_USEGRPC={use_grpc}']
                )
