...
def main():
    parser = argparse.ArgumentParser(
        description="Report test execution results in structured format"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        help='Test execution status'
    )
    
    parser.add_argument(
        '--reason',
        help='Reason for failure or skip (optional for passed tests)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Number of skipped steps'
    )
    
    parser.add_argument(
        '--duration',
        help='Test duration (e.g., "2m 30s" or "150s")'
    )
    
    parser.add_argument(
        '--error-details',
        help='Detailed error information (optional)'
    )
    
    parser.add_argument(
        '--step-details',
        help='JSON string with step-by-step execution details (optional)'
    )
    
    parser.add_argument(
        '--issues',
        help='JSON string with issues/errors encountered (optional)'
    )
