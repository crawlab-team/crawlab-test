...
class ParallelTestExecutor:
    ...
    def execute_specs(self, spec_paths: List[Path], config: Dict) -> List[Dict]:
        ...
        if len(spec_paths) == 1:
            print(f"Single spec detected, running sequentially")
        ...
        if effective_workers < self.max_workers:
            print(f"Executing {len(spec_paths)} specs with {effective_workers} parallel workers (capped from {self.max_workers} to match test count)")
        else:
            print(f"Executing {len(spec_paths)} specs with {effective_workers} parallel workers")
        print(f"Workers will run in separate processes for isolation")
        print("")
        ...
        with ProcessPoolExecutor(max_workers=effective_workers) as executor:
            # Submit all tasks
            future_to_spec = {
                executor.submit(_execute_single_spec, work_item): work_item[0]
